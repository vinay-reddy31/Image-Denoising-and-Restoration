...
Original file is located at
    https://colab.research.google.com/drive/1CqZygGkoZ_RpGF5GMVyluKe8XAeCYCmd
"""
...
# Apply bicubic interpolation to upscale the images
def apply_bicubic_interpolation(lr_images, original_shape):
    hr_images = np.zeros(original_shape)
    for i in range(lr_images.shape[0]):
        hr_img = resize(lr_images[i], original_shape[1:], anti_aliasing=True, order=3)  # order=3 for bicubic interpolation
        hr_images[i] = hr_img
    return hr_images

X_train_bicubic = apply_bicubic_interpolation(X_train_lr, X_train.shape)
X_test_bicubic = apply_bicubic_interpolation(X_test_lr, X_test.shape)
...
# Evaluate the bicubic interpolation method
psnr_values = []
ssim_values = []
lpips_values = []
...
df = pd.DataFrame(data, index=['Bicubic'])

# Save the DataFrame to an Excel file
df.to_excel('metrics_bicubic.xlsx')
...
for i in range(5):
    show_images(X_test_lr[i], X_test_bicubic[i], X_test[i])
